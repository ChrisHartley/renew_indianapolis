...
def searchSurplusProperties(request):
    ...
    if request.GET.get('report') == 'false':
        qs = Parcel.objects.exclude(area__lte=500).filter(Q(requested_from_commissioners_date__exact='2018-08-16') & Q(requested_from_commissioners__exact=True)).exclude(parcel_number__in=properties_with_reports).exclude(intended_end_use='BEP')
    else:
        qs = Parcel.objects.exclude(area__lte=500).filter(Q(requested_from_commissioners_date__exact='2018-08-16') & Q(requested_from_commissioners__exact=True)).filter(parcel_number__in=properties_with_reports).exclude(intended_end_use='BEP')
